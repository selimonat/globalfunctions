function fixmat = getfixmat_raw(edfdata, edfmeta)
            ...
            %
            %   Based on getfixmat, imports the raw data instead of
            %   fixation points, 21.10.2016
...
%                 fixmat.stop  = zeros(0, 0, 'int32');
            ...
            for trial=(1:trials)
                ...
                fix = edfdata(trial).(eyes{want_eye(trial)}).samples;
                
                fix_in_trial = length(fix.x);
                ...
                fixmat.x     = [fixmat.x, fix.x];
                fixmat.y     = [fixmat.y, fix.y];
                fixmat.time  = [fixmat.time, fix.time];
                fixmat.triali= [fixmat.triali,repmat(trial,1,fix_in_trial)];
                if with_pupil
                    fixmat.pupil = [fixmat.pupil, fix.pupil];
                end
            end;
            % add user defined meta data
            for f=(1:exp_fields_length)
                fixmat.(exp_fields{f}) = ones(1, length(fixmat.x), 'int32') * str2double(edfmeta.(exp_fields{f}));
            end
